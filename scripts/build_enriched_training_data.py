...
# Synced with clean_context_stream_promo.py PROMO_REMOVE_PATTERNS + TC_PROMO_RE
PROMO_RE = re.compile(
    r'카카오\s*(?:톡\s*)?(?:아이디|ID)?\s*[:：\s]\s*[A-Za-z0-9_\-]{3,}|'
    r'카톡\s*(?:아이디|ID)?\s*[:：\s]\s*[A-Za-z0-9_\-]{3,}|'
    r'카카오\s+[a-zA-Z][a-zA-Z0-9]{2,}|카톡\s+[a-zA-Z][a-zA-Z0-9]{2,}|'
    r'카카오톡?\s*(?:아이디|ID)|♡카카|카톡 [a-z]|'
    r'텔레?\s*[:：]\s*@?\S{3,20}|라인\s*[:：]\s*\S{3,20}|'
    r'오픈\s*(?:채팅|카톡|톡)|밀빵\s*(?:확실|가능|보장)|'
    r'풀\s*(?:상주|케어)|출근\s*문의|'
    r'안녕하세요[!.]|좋은\s*하루|문의\s*사항|편하게\s*연락|상세히\s*알려드릴|'
    r'(?:T\.?C|티씨|팁제).{0,30}(?:문의|연락|카[카톡]|실장|출근)|'
    r'(?:지명비|보장|일급|시급)\s*\d+\s*(?:만원|만)|'
    r'문의\s*(?:주|하)\s*(?:세요|십시오|시면)|'
    r'(?:출퇴근|출근부터|퇴근까지).{0,15}(?:케어|관리|안전)',
    re.IGNORECASE
)

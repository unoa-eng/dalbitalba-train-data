...
GATE_V2 = {
    ...
    "cross_machine_agreement": ("le", 0.10),     # JSD-like, smaller is better
}
...
def persona_consistency(ai_rows: list[dict], persona_list_path: Path | None) -> tuple[float, dict]:
    if not persona_list_path or not persona_list_path.exists():
        return 1.0, {"skipped": "no persona-list provided"}
    ...
    for r in ai_rows:
        ...
        if pid in accepted_ids or pname in accepted_names:
            matched += 1
    if not declared:
        return 1.0, {"skipped": "no persona-tagged rows"}
...
def cross_machine_agreement(ai_rows: list[dict], secondary_rows: list[dict]) -> tuple[float, dict]:
    ...
    if not secondary_rows:
        return 0.0, {"skipped": "no secondary-ai provided"}
...
def main() -> int:
    ...
    ai_rows = base.load_rows(args.ai)
    raw_rows = base.load_rows(args.raw)
    secondary_rows = base.load_rows(args.secondary_ai) if args.secondary_ai else []
